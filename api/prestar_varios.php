<?php
// Archivo: api/prestar_varios.php
...
// Usamos una transacción para asegurar que todo o nada se preste
try {
    ...
    // Preparamos las consultas una sola vez fuera del bucle
    ...
    foreach ($id_libros as $id_libro) {
        // 1. Verificamos si ya está prestado por este usuario
        $stmt_check->execute([':id_libro' => $id_libro, ':id_usuario' => $id_usuario]);
        
        if (!$stmt_check->fetch()) {
            // 2. Si no lo tiene, lo insertamos
            $stmt_insert->execute([
    ...
    // Algo salió mal, revertimos toda la operación
    $conn->rollBack();
